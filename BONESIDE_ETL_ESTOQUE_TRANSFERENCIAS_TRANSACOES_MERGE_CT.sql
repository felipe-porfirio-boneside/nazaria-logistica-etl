CREATE PROCEDURE [dbo].[BONESIDE_ETL_ESTOQUE_TRANSFERENCIAS_TRANSACOES_MERGE_CT] 
(
...
using (
			...
			where t.name = 'ESTOQUE_TRANSFERENCIAS_TRANSACOES'
		) AS source
...
using (
			...
			inner join PBS_NAZARIA_DADOS.sys.schemas s 
				on s.schema_id = t.schema_id
			where t.name = 'ESTOQUE_TRANSFERENCIAS'
		) AS source
	ON target.Table_Name = source.Table_Name
